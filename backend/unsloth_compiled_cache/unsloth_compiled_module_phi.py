...
from transformers.models.phi.modeling_phi import (partial, Callable, Optional, Tuple, Union, torch, nn, ACT2FN, Cache, GenerationMixin, BaseModelOutputWithPast, CausalLMOutputWithPast, ROPE_INIT_FUNCTIONS, dynamic_rope_update, ALL_ATTENTION_FUNCTIONS, PreTrainedModel, Unpack, add_start_docstrings, add_start_docstrings_to_model_forward, can_return_tuple, replace_return_docstrings, deprecate_kwarg, PhiConfig, logger, __name__, _CONFIG_FOR_DOC, PhiPreTrainedModel, PHI_INPUTS_DOCSTRING, PhiModel, KwargsForCausalLM, PhiForCausalLM)
...
class PhiAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        return PhiAttention_forward(self, hidden_states, position_embeddings, attention_mask, past_key_value, cache_position, **kwargs)


@torch.compile(fullgraph = False, dynamic = True, options = torch_compile_options)
def PhiMLP_forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
...
class PhiMLP(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        return PhiMLP_forward(self, hidden_states)


@torch.compile(fullgraph = True, dynamic = True, options = torch_compile_options)
@torch.no_grad()
@dynamic_rope_update  # power user: used with advanced RoPE types (e.g. dynamic rope)
def PhiRotaryEmbedding_forward(self, x, position_ids):
...
class PhiRotaryEmbedding(nn.Module):
    ...
    def forward(self, x, position_ids):
        return PhiRotaryEmbedding_forward(self, x, position_ids)
...
class PhiForCausalLM(PhiPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        **kwargs: Unpack[KwargsForCausalLM],
        ...
        return PhiForCausalLM_forward(self, input_ids, attention_mask, position_ids, past_key_values, inputs_embeds, labels, use_cache, output_attentions, output_hidden_states, cache_position, logits_to_keep, **kwargs)
